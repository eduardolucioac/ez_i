...
F_EZ_READ_R=""
f_ez_read() {
    : 'Facilitar o uso do comando "read".

    Trata algumas limitações do comando "read".
    ...
    password. Padrão 0.
        WAIT_UNTIL_P (Optional[int]): Esperar até o intervalo informado 
            (em segundos). Caso seja vazio irá esperar uma resposta indefinidamente.
        WAIT_UNTIL_RTN_P (Optional[str]): Valor a ser assumido após o intervalo 
            em WAIT_UNTIL_P.

    Returns:
        F_EZ_READ_R (str): Entrada digitada pelo usuário.
    '
    ...
    # NOTE: Habilita o "modo password". By Questor

        READ_PARAMS="-s "
    fi
    if [ -n "$WAIT_UNTIL_P" ] ; then
    # NOTE: Habilita o "timeout". By Questor

        READ_PARAMS="$READ_PARAMS-t$WAIT_UNTIL_P "
        QUESTION_P="$QUESTION_P (\"$WAIT_UNTIL_RTN_P\" in $WAIT_UNTIL_P seconds) "
    fi

    # NOTE: Exibimos de forma separada o conteúdo de "QUESTION_P" usando o comando
    # "echo" e não usamos o comando "read" para exibi-lo porque "strings" muito longas
    # ocasionam o erro "Segmentation fault (core dumped)" na execução do comando
    # "read" além de outros problemas de funcionamento. By Questor
    echo -n "$QUESTION_P"
    eval "read -e $READ_PARAMS-r F_EZ_READ_R"

    # NOTE: Os "echo" abaixo serão necessários se a resposta NÃO for vazia e o "modo
    # password" for habilitado ou se a resposta for vazia e o "modo timeout" for
    # habilitado ou se a resposta for vazia e nem o "modo password" e nem o "modo
    # timeout" estiverem habilitados. Evita efeitos colaterais graficos indesejados
    # gerados pelo comando "read" nestas circunstâncias. By Questor
    ...
    # NOTE: Se a resposta for vazia e o "timeout" for habilitado, então a resposta
    # padrão em "WAIT_UNTIL_RTN_P" será retornada. By Questor
